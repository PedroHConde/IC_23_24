...
//    digitalWrite(LED_BUILTIN, HIGH);  // Acende LED_BUILTIN (piloto; indicador de atividade do Arduino)
  ...
  if (Serial.available() >= 6) { // Verifica se há pelo menos 6 bytes disponíveis
    ...
    if (startByte == 0) { // Byte de início
      byte taskByte = Serial.read(); // Byte para a tarefa (switch case)
      byte dataBytes[3];

      for (int i = 0; i < 3; i++) {
        dataBytes[i] = Serial.read(); // Lê os 3 bytes de dados
      }

      byte endByte = Serial.read(); // Byte de fim

      if (endByte == 1) { // Verifica se o byte de fim está correto
        // Processa a tarefa com base no valor de taskByte
        switch (taskByte) {
          case 1: 
          if(!malhaFechada){// Duty Cycle [%]. Válido somente em MA
          recebeDC = !recebeDC;
          }
          case 2: 
        ...
        case 3:
        if (malhaFechada) { // Controle tipo Liga/Desliga ao invés do PID. Válido somente em MF
          ligaDesliga = !ligaDesliga;
        }
        case 4:
        if (malhaFechada) { // Operação em MA
          modoNeutro();
          malhaFechada = false;
        }
        case 5:
       ...
       case 6:
        // Modo manual de referência de temperatura
        ...
        case  8: // Configuração das coisas
          //duracaoDegrau = comando.substring(0, 4).toInt();
          //numDegraus =comando.substring(5, 6).toInt();
          //recebeConfigDeg = false;
        
        case 10://Desliga tudo
          modoNeutro();
        
        case 11 : 
        if (((malhaFechada && recebeTempRef) || (!malhaFechada && recebeDC)) && stringInteiroComSinal(comando)) {
          int valor = 0;
